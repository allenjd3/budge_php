      ...
      <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
          ...
          <template v-if="c.items">
            <item-component v-for="item in c.items" :key="item.id" :name="item.name" :planned="item.planned" :spent="item.remaining" :monthId="month.id" @toggleModal="createModifyItem(item)"/>
          </template>
        </div>
      </div>
    </div>
